...
import { getListingDetailsPk, updateListing } from '../../apiRequest/actions/listingActions';
...
const EditPropertyScreen = ({pk, showModal, handleClose }) => {
  ...
  const {error: errorUpdate, loading: loadingUpdate, listing: updatedlisting, success} = listingUpdate
...
const handleSubmit = (e) => {
  ...
  return (
    <>
    {loading ? <Loader /> : error ? <Message1 variant='danger'>{error}</Message1>
      : (listing && listing.title && listing.main_photo && listing.address && 
      ...
      <Modal.Body >
        ...
        <Row>
          ...
          <Form.Group controlId="main_photo">
            ...
            <div>
              {listing.main_photo && (
              <img src={listing.main_photo} alt="Main Photo" style={{height: '6rem', width: '8rem'}}/>
              )}
            </div>
          </Form.Group>
                ...
                {(<>
                  {listing.images && (
                    uploaded_images.map((img, index) => (
                      <div className={uploaded_images.length < 4 ? "col-md-3" : "col"} key={index} style={uploaded_images.length <= 4 ? { height: '8rem'} : { height: '6rem' , width: '4rem'}}>
                        <img src={img.image} alt={`Preview ${index}`} />
                        <Button variant="danger" onClick={() => handleImageDelete(index)}>Delete</Button>
                      </div>
                    ))
                  )}
